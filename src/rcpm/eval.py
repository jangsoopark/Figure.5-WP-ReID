...
class RecurrentContextPropagationModule(object):
    ...
    def rcpm(self, gt_dist_new, iteration=1):
        distMat_new = visual_affinity_update(
            self.raw_distMat, gt_dist=gt_dist_new.copy(), T=self.delta, alpha=self.alpha
        )
        cmc_reid, mAP_reid = compute_video_cmc_map(
            distMat_new[self.query_index][:, self.gallery_index], self.query_id, self.gallery_id,
            self.query_cam_id, self.gallery_cam_id
        )
        gt_dist_new = trajectory_distance_update(distMat_new, self.raw_gt_dist.copy(), k=self.k)
        cmc_SM = get_signal_match_cmc(gt_dist_new[self.query_index].copy(),
        ...
        print('Iteration {}: ReID rank-1 {:.2f} mAP {:.2f}. Signal Matching rank-1 {:.2f}'.format(
            iteration, cmc_reid[0] * 100, mAP_reid * 100, cmc_SM[0] * 100))
        return gt_dist_new

    def __call__(self, *args, **kwargs):
        print('K={}, Delta={}, Iteration={}'.format(self.k, self.delta, self.iters))
        cmc_reid, mAP_reid = compute_video_cmc_map(self.raw_distMat[self.query_index][:, self.gallery_index],
                                                   self.query_id, self.gallery_id,
                                                   self.query_cam_id, self.gallery_cam_id)
        cmc_SM = get_signal_match_cmc(self.raw_gt_dist[self.query_index].copy(),
                                      self.gt_dist_is_inf[self.query_index].copy(),
                                      self.query_id.copy(), self.traj_id.copy())
...
def update_with_gps(distMat, all_info, query_index, gallery_index, bndbox_gps, trajectory, k=9, delta=74, iters=5):
    ...
    rcpm = RecurrentContextPropagationModule(k=k, delta=delta, iters=iters, distMat=distMat, gt_dist=gt_gps_dist,
                                             traj_id=traj_id)
